...
public class MeldableHeap extends MeldablePQ implements ClickListener {
    public static final String dsName = "meldableheap";
    MeldableHeapNode root[] = null;

    public MeldableHeap(VisPanel M) {
        super(M);
        root = new MeldableHeapNode[numHeaps + 1];
    ...
    public void mouseClicked(int x, int y) {
        int h = 0;
        MeldableHeapNode v = null;
        for (int i = 1; i <= numHeaps; ++i) {
            if (root[i] != null) {
                v = (MeldableHeapNode) root[i].find(x, y);
                if (v != null) {
                    h = i;
                    break;
                }
    ...
    public void insert(int x) {
        start(new MeldableHeapInsert(this, x));

    }

    @Override
    public void delete() {
        start(new MeldableHeapDelete(this));
    }
    ...
    public void meld(int i, int j) {
        ...
        start(new MeldableHeapMeld(this, i, j));
    ...
    public void decreaseKey(Node v, int delta) {
        if (v == null) {
            ...
            start(new MeldableHeapDecrKey(this, (MeldableHeapNode) v, delta));
        }
    }
    ...
    public void restoreState(Hashtable<?, ?> state) {
        ...
        if (root != null) {
            this.root = (MeldableHeapNode[]) HashtableStoreSupport.restore(root);
        }
        for (int i = 0; i <= numHeaps; ++i) {
            if (this.root[i] != null) {
