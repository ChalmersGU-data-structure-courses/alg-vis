...
public class MeldableHeapNode extends BSTNode {
    private boolean doubleArrow = false;
    boolean dashedRightLine = false; // if true the line leading to the right
                                    // son is dashed
    boolean dashedLeftLine = false; // if true the line leading to the
                                           // left son is dashed

    private MeldableHeapNode(DataStructure D, int key, int x, int y) {
        super(D, key, x, y);
        bgKeyColor();
    }

    public MeldableHeapNode(DataStructure D, int key, int zDepth) {
        super(D, key, zDepth);
        bgKeyColor();
    }

    public MeldableHeapNode(MeldableHeapNode v) {
    ...
    public void linkup(MeldableHeapNode v) {
        if ((this.getParent() != null) && (v != null)) {
            final MeldableHeapNode tmp = this.getParent();
            v.setRight(this);
            this.setParent(v);
            v.setParent(tmp);
    ...
    public void swapChildren() {
        final MeldableHeapNode tmp = this.getLeft();
    ...
    public void repos(int px, int py) {
        ...
        if (this.getRight() != null) {
            this.getRight().repos(px + getRight().leftw,
                py + (DataStructure.minsepy));// + 2 * MeldableHeapNode.RADIUS));
        }
    ...
    @Override
    public MeldableHeapNode getRight() {
        return (MeldableHeapNode) super.getRight();
    }

    public void setRight(MeldableHeapNode v) {
        super.setRight(v);
    }

    @Override
    public MeldableHeapNode getLeft() {
        return (MeldableHeapNode) super.getLeft();
    }

    void setLeft(MeldableHeapNode v) {
        super.setLeft(v);
    }

    @Override
    public MeldableHeapNode getParent() {
        return (MeldableHeapNode) super.getParent();
    }

    public void setParent(MeldableHeapNode v) {
        super.setParent(v);
    }
